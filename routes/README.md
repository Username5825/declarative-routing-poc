...
| `/api/pokemon` | GET | `PokemonSearchAPI` | `getPokemonSearchAPI(...)` |
| `/` | - | `Home` | `<Home.Link>` |
| `/pokemon/[pokemonId]` | - | `PokemonDetail` | `<PokemonDetail.Link>` |
...
- [ ] Convert `GET` fetch calls to `/api/pokemon` to `getPokemonSearchAPI(...)` calls
...
- [ ] Convert `Link` components for `/pokemon/[pokemonId]` to `<PokemonDetail.Link>`
